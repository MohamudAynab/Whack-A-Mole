...
let time = 0;
let timer;
let lastHole = null; // will always be false, because you're comparing a DOM element with a number (0).
let points = 0;
let difficulty = 'hard';
...
function chooseHole(holes) {
	// TODO: Write your code here.
	const index = randomInteger(0, holes.length - 1); // pick a random number
	const hole = holes[index]; // get the hole at that index

	if (hole === lastHole) {
		return chooseHole(holes); // try again if same as last one
	}

	lastHole = hole; // remember it
	return hole; // return the new one
}
...
function gameOver() {
	// TODO: Write your code here
	if (time > 0) {
		const timeoutId = showUp(); // continue the game
		return timeoutId;
	} else {
...
function updateScore() {
	// TODO: Write your code here
	points += 1; // Increment the global points variable
	score.textContent = points; // Update the scoreboard in the DOM
	return points;
}
...
function clearScore() {
	// TODO: Write your code here
	points = 0;
	score.textContent = points; // Update the dom display
	return points; // return the reset score to 0
}
...
function updateTimer() {
	...
	if (time > 0) {
		time -= 1; // Decrease the time by 1 second
		timerDisplay.textContent = time; // update the timer on the page
	}
